...
import com.tagme.R
import com.tagme.domain.models.MessageData
...
class ConversationFragment : Fragment(), MessageAdapter.LastMessageIdListener {
    ...
    override fun onCreateView(
        ...
        CoroutineScope(Dispatchers.Main).launch {
            val conversation =  viewModel.getMessagesFromWS(conversationId)
        ...
        sendMessageButton.setOnClickListener {
            ...
            CoroutineScope(Dispatchers.Main).launch {
                val answer = viewModel.sendMessageToWS(conversationId, text)
                if (answer != null && answer.getString("status") == "success") {
                    val message = answer.getString("message")
                    if (message == "success") {
                        val updatedMessages = viewModel.getMessagesFromWS(conversationId)?.messages.orEmpty()
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        messageUpdateRunnable = Runnable {
            CoroutineScope(Dispatchers.Main).launch {
                val conversationId = requireArguments().getInt(ARG_CONVERSATION_ID)
                val updatedMessages = viewModel.getMessagesFromWS(conversationId)?.messages
...
class MessageAdapter(
    private var messageList:MutableList<MessageData>,
    ...
    fun updateData(newMessages: MutableList<MessageData>) {
