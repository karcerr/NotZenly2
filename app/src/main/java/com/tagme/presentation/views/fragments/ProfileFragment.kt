...
import com.tagme.domain.models.FriendRequestData
...
class ProfileFragment : Fragment() {
    ...
    override fun onCreateView(
        ...
        changeNicknameButton2.setOnClickListener{
            ...
            coroutineScope.launch {
                val answer = viewModel.changeNickname(nickname)
                if (answer != null) {
                    val message = answer.getString("message")
                    if (answer.getString("status") == "success") {
                        viewModel.getMyDataWS()
                        nicknameChangeStatusText.visibility = View.INVISIBLE
        ...
        myProfilePic.setOnClickListener {
            ...
            coroutineScope.launch {
                ...
                if (viewModel.updatePfpPic(imageHandler)) {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        friendRequestUpdateRunnable = Runnable {
            CoroutineScope(Dispatchers.Main).launch {
                viewModel.updateFriendRequestsAndFriendsWS()
                val updatedFriendRequests = viewModel.getFriendRequestDataList()
        ...
        when (holder.itemViewType) {
            VIEW_TYPE_INCOMING -> {
                ...
                incomingHolder.acceptButton.setOnClickListener { view ->
                    ...
                    incomingHolder.coroutineScope.launch {
                        val answer = viewModel.acceptFriendRequest(userId)
                        if (answer != null) {
                            if (answer.getString("status") == "success") {
                ...
                incomingHolder.denyButton.setOnClickListener { view ->
                    val userId = view.tag as Int
                    incomingHolder.coroutineScope.launch {
                        val answer = viewModel.denyFriendRequest(userId)
                        if (answer != null) {
                            if (answer.getString("status") == "success") {
                                removeItem(holder.adapterPosition)
                                friendAdapter.updateData(viewModel.getFriendDataList())
                            }
                        }
                    }
                }
            ...
            VIEW_TYPE_OUTGOING -> {
                ...
                outgoingHolder.cancelButton.setOnClickListener { view ->
                    val userId = view.tag as Int
                    outgoingHolder.coroutineScope.launch {
                        val answer = viewModel.cancelFriendRequestWS(userId)
                        if (answer != null) {
                            if (answer.getString("status") == "success") {
                                removeItem(holder.adapterPosition)
                                friendAdapter.updateData(viewModel.getFriendDataList())
